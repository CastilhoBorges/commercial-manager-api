...
const data = {
  namePascalCase: toPascalCase(name), // ExemploName,
  nameCamelCase: toCamelCase(name), // ExemploName,
  nameKebabCase: toKebabCase(name), // exemplo-name,
  nameSnakeCase: toSnakeCase(name), // exemplo-name,
};
...
function generateTemplate(folder, file) {
  const templatePath = path.join(
    __dirname,
    `module/${version}/${folder ? `${folder}/` : ''}${file}.hbs`,
  );
  ...
  const outputDir = path.join(
    __dirname,
    src,
    data.nameKebabCase.toLowerCase(),
    folder || '',
  );
  const outputPath = path.join(
    outputDir,
    `${data.nameKebabCase.toLowerCase()}.${file}.ts`,
  );
...
function generateAllTemplates() {
  ...
  if (!folderExists(folderPath)) {
    try {
      const basePath = path.join(__dirname, `./module/${version}`);
